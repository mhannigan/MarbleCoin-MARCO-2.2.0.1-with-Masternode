...
CKeePassIntegrator* pKeePassInt = NULL;
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: Marble.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: Marbled.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeMarbleamount=<n> " + _("Keep N Marble anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    nMagicBlock = GetArg("-magicblock", 100000);
    
    LogPrintf("Magic block is set to %d\n", nMagicBlock);
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Marble is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Marble is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Marble version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (!fDisableWallet) {
        ...
        pKeePassInt = new CKeePassIntegrator();
        pKeePassInt->init();
    } // (!fDisableWallet)
#endif // ENABLE_WALLET
    ...
    nAnonymizeMarbleAmount = GetArg("-anonymizeMarbleamount", 0);
    if(nAnonymizeMarbleAmount > 999999) nAnonymizeMarbleAmount = 999999;
    if(nAnonymizeMarbleAmount < 2) nAnonymizeMarbleAmount = 2;
    ...
    if(fEnableInstantX){
        ...
        if(nInstantXDepth < 0) nAnonymizeMarbleAmount = 0;
    } else {
        nInstantXDepth = 0;
    }
    ...
    LogPrintf("Anonymize Marble Amount %d\n", nAnonymizeMarbleAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1Marble+1000 == (.1Marble+100)*10
       10Marble+10000 == (1Marble+1000)*10
    */
