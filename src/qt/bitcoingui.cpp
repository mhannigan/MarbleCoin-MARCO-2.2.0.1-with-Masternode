    ...
    nWeight(0)
{
    ...
    labelMasterNodeMode = new QLabel();
    ...
    frameBlocksLayout->addWidget(labelMasterNodeMode);
    frameBlocksLayout->addStretch();
...
void BitcoinGUI::createActions()
{
    ...
    masternodeManagerAction->setToolTip(tr("Show Marble Nodes status and configure your nodes."));
    ...
    aboutAction = new QAction(tr("&About Marble"), this);
...
void BitcoinGUI::createToolBars()
{
    ...
    //menuAction->setToolTip(tr("Access Marble Wallet Tabs"));
...
void BitcoinGUI::updateMasterNodeModeIcon()
{
    int nHeight = clientModel->getNumBlocks();

    //LogPrintf("BitcoinGUI::updateMasterNodeModeIcon()\n");
    if(IsProtocolV2(nHeight)) {
        //check is masternode active
        if(activeMasternode.status == MASTERNODE_IS_CAPABLE || activeMasternode.status == MASTERNODE_REMOTELY_ENABLED) {
            labelMasterNodeMode->setPixmap(QIcon(":/icons/masternode_green").pixmap(STATUSBAR_ICONSIZE,STATUSBAR_ICONSIZE));
            labelMasterNodeMode->setToolTip(tr("Marble protocol 2.0, height %1, masternode is active").arg(nHeight));
            //LogPrintf("BitcoinGUI::updateMasterNodeModeIcon(), green, height is %d\n",nHeight);
        } else {
            labelMasterNodeMode->setPixmap(QIcon(":/icons/masternode_yellow").pixmap(STATUSBAR_ICONSIZE,STATUSBAR_ICONSIZE));
            labelMasterNodeMode->setToolTip(tr("Marble protocol 2.0, height %1, masternode is not active").arg(nHeight));
            //LogPrintf("BitcoinGUI::updateMasterNodeModeIcon(), yellow, height is %d\n",nHeight);
        }

    }
    else {
    	labelMasterNodeMode->setPixmap(QIcon(":/icons/masternode_gray").pixmap(STATUSBAR_ICONSIZE,STATUSBAR_ICONSIZE));
        labelMasterNodeMode->setToolTip(tr("Marble protocol 1.0, height %1").arg(nHeight));
        //LogPrintf("BitcoinGUI::updateMasterNodeModeIcon(), gray, height is %d\n",nHeight);
   }
}
