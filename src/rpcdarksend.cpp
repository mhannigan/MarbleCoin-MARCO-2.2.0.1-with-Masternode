...
Value darksend(const Array& params, bool fHelp)
{
    if (fHelp || params.size() == 0)
        throw runtime_error(
            "darksend <Marbleaddress> <amount>\n"
            "Marbleaddress, reset, or auto (AutoDenominate)"
    ...
    if (params.size() != 2)
        throw runtime_error(
            "darksend <Marbleaddress> <amount>\n"
            "Marbleaddress, denominate, or auto (AutoDenominate)"
            "<amount> is a real and is rounded to the nearest 0.00000001"
            + HelpRequiringPassphrase());
...
Value masternode(const Array& params, bool fHelp)
{
    ...
    if (strCommand == "start")
    {
        //disable MN start before the magic block
        if(!IsProtocolV2(pindexBest->nHeight)) 
		return strprintf("It is not possible to start masternode before reaching the block %d", nMagicBlock);
    ...
    if (strCommand == "winners")
    {
        ...
        //disable MN start before the magic block
        if(!IsProtocolV2(pindexBest->nHeight)) 
		return strprintf("It is not possible to get masternode winners before reaching the block %d", nMagicBlock);
    ...
    if(strCommand == "connect")
    {
        //disable MN start before the magic block
        if(!IsProtocolV2(pindexBest->nHeight)) 
		return strprintf("It is not possible to connect masternode before reaching the block %d", nMagicBlock);
