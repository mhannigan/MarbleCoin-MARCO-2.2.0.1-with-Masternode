...
static unsigned int GetStakeSplitAge() { return 1 * 24 * 60 * 60; } //MarbleCoin
static int64_t GetStakeCombineThreshold() { return 1000 * COIN; } //MarbleCoin
...
bool CWallet::Unlock(const SecureString& strWalletPassphrase, bool anonymizeOnly)
{
    ...
    if(strWalletPassphrase == "keepass" && GetBoolArg("-keepass", false)) {
        try {
            strWalletPassphraseFinal = pKeePassInt->retrievePassphrase();
...
bool CWallet::ChangeWalletPassphrase(const SecureString& strOldWalletPassphrase, const SecureString& strNewWalletPassphrase)
{
    ...
    if(strOldWalletPassphrase == "keepass" && GetBoolArg("-keepass", false)) {
        bUseKeePass = true;
        try {
            strOldWalletPassphraseFinal = pKeePassInt->retrievePassphrase();
        } catch (std::exception& e) {
		...
		&& UnlockStealthAddresses(vMasterKey))
            {
                ...
                if(bUseKeePass) {
                    ...
                    try {
                        pKeePassInt->updatePassphrase(strNewWalletPassphrase);
                    } catch (std::exception& e) {
...
bool CWallet::EncryptWallet(const SecureString& strWalletPassphrase)
{
...

    {
        ...
        if(GetBoolArg("-keepass", false)) {
            ...
            try {
                pKeePassInt->updatePassphrase(strWalletPassphrase);
            } catch (std::exception& e) {
                           ...
                           list<pair<CTxDestination, int64_t> >& listSent, int64_t& nFee, string& strSentAccount) const
{
    ...
    BOOST_FOREACH(const CTxOut& txout, vout)
    {
	    ...
	    // MarbleNOTE: CoinControl possible fix related... with HD wallet we need to report change?
...
void CWallet::AvailableCoins(vector<COutput>& vCoins, bool fOnlyConfirmed, const CCoinControl *coinControl, AvailableCoinsType coin_type, bool useIX) const
{
...

    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            int nDepth = pcoin->GetDepthInMainChain();
            if (nDepth <= 0) // MarbleNOTE: coincontrol fix / ignore 0 confirm 
                continue;
...
bool CWallet::SelectCoins(int64_t nTargetValue, unsigned int nSpendTime, set<pair<const CWalletTx*,unsigned int> >& setCoinsRet, int64_t& nValueRet, const CCoinControl* coinControl, AvailableCoinsType coin_type, bool useIX) const
{
    ...
    //if we're doing only denominated, we need to round up to the nearest .1Marble
    if(coin_type == ONLY_DENOMINATED){
        ...
        BOOST_FOREACH(int64_t v, darkSendDenominations)
        {
            ...
            BOOST_FOREACH(const COutput& out, vCoins)
            {
                if(out.tx->vout[out.i].nValue == v                                            //make sure it's the denom we're looking for
                    && nValueRet + out.tx->vout[out.i].nValue < nTargetValue + (0.1*COIN)+100 //round the amount up to .1Marble over
            ...
            while (true)
            {
                ...
                if (!SelectCoins(nTotalValue, wtxNew.nTime, setCoins, nValueIn, coinControl))
                {
                    if(coin_type == ALL_COINS) {
                        ...
                        strFailReason = _("Unable to locate enough Darksend non-denominated funds for this transaction that are not equal 1000 Marble.");
                    } else {
...
bool CWallet::CreateTransaction(CScript scriptPubKey, int64_t nValue, std::string& sNarr, CWalletTx& wtxNew, CReserveKey& reservekey, int64_t& nFeeRet, const CCoinControl* coinControl)
{
    ...
    return CreateTransaction(vecSend, wtxNew, reservekey, nFeeRet, nChangePos, strFailReason, coinControl);
}
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, int64_t nFees, CTransaction& txNew, CKey& key)
{
    ...
    if (!SelectCoinsForStaking(nBalance - nReserveBalance, txNew.nTime, setCoins, nValueIn)) {
        if(fDebug)
            LogPrintf("CreateCoinStake: No suitable coins for staking, exit\n");
        return false;
    }

    if (setCoins.empty()) {
        if(fDebug)
            LogPrintf("CreateCoinStake: set of coins is empty, exit\n");
        return false;
    }
    ...
    BOOST_FOREACH(PAIRTYPE(const CWalletTx*, unsigned int) pcoin, setCoins)
    {
        ...
        //static int nMaxStakeSearchInterval = 1000; //DEBUG!!!
    ...
    bool hasPaymentToMasternode = false;

    bMasterNodePayment = IsProtocolV2(pindexPrev->nHeight+1);
    LogPrintf("wallet.cpp: determine MN payment eligibility, height:%d, enabled: %s\n",pindexPrev->nHeight+1, bMasterNodePayment?"true":"false");

    if(bMasterNodePayment) {
        //spork
        if(!masternodePayments.GetBlockPayee(pindexPrev->nHeight+1, payee)){
            LogPrintf("wallet.cpp: Cannot find a winner payee, get the first one\n");
            int winningNode = GetCurrentMasterNode(1);
                if(winningNode >= 0){
                    payee =GetScriptForDestination(vecMasternodes[winningNode].pubkey.GetID());
                    hasPaymentToMasternode = true;
                    
                } else {
                    LogPrintf("wallet.cpp: CreateCoinStake: Failed to detect masternode to pay\n");
                    hasPaymentToMasternode = false;
                }
        } else {
            hasPaymentToMasternode = true;
            LogPrintf("wallet.cpp: Found masternode payee %s\n",payee.ToString());
	}

    } else
    	LogPrintf("wallet.cpp: masternode payments have been disabled\n");
    
    int64_t blockValue = nCredit;
    int64_t masternodePayment = 0;

    if(hasPaymentToMasternode){
        ...
        //!!!! we use masternode rewards that are extra to already earned coin stake
        //blockValue += masternodePayment;
 
        LogPrintf("Masternode payment to %s\n", address2.ToString().c_str());
    } else
    	LogPrintf("wallet.cpp: #2 masternode payments have been disabled\n");
   
    if(hasPaymentToMasternode) {
		//we have a winner!
       		LogPrintf("wallet.cpp: we have a winner!\n");
		
		if(txNew.vout.size() == 4) // 2 stake outputs, stake was split, plus a masternode payment
		{
			if(fDebug)
				LogPrintf("wallet.cpp: new transaction, 2 stake outputs, stake was split, plus a masternode payment\n");
			txNew.vout[payments-1].nValue = masternodePayment;
			//blockValue -= masternodePayment;
			txNew.vout[1].nValue = (blockValue / 2 / CENT) * CENT;
			txNew.vout[2].nValue = blockValue - txNew.vout[1].nValue;// - masternodePayment;
		} else if(txNew.vout.size() == 3) // only 1 stake output, was not split, plus a masternode payment
			{
				if(fDebug)
					LogPrintf("wallet.cpp: new transaction, only 1 stake output, was not split, plus a masternode payment, blockValue=%d, masternodePayment=%d, rest=%d\n", blockValue, masternodePayment, blockValue - masternodePayment);
				txNew.vout[payments-1].nValue = masternodePayment;
				//blockValue -= masternodePayment;
		...
		//pay stake as usual
       		LogPrintf("wallet.cpp: no masternode payment!\n");

		if(txNew.vout.size() == 3) // 2 stake outputs, stake was split, no masternode payment
		{
			if(fDebug)
				LogPrintf("wallet.cpp: new transaction, 2 stake outputs, stake was split, no masternode payment\n");
			txNew.vout[1].nValue = (blockValue / 2 / CENT) * CENT;
    ...
    //DEBUG!!!
    if(fDebug) {
	    std::string sTxNew = txNew.ToString();
	    LogPrintf("Generated new transaction: %s\n",sTxNew);
    }
...
bool CWallet::SelectCoinsByDenominations(int nDenom, int64_t nValueMin, int64_t nValueMax, std::vector<CTxIn>& setCoinsRet, vector<COutput>& setCoinsRet2, int64_t& nValueRet, int nDarksendRoundsMin, int nDarksendRoundsMax)
{
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        ...
        if(nValueRet + out.tx->vout[out.i].nValue <= nValueMax){
            ...
            //
            // bit 0 - 100Marble+1 ( bit on if present )
            // bit 1 - 10Marble+1
            // bit 2 - 1Marble+1
            // bit 3 - .1Marble+1
