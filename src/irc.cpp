...
void ThreadIRCSeed2()
{
	...
	int ntimeout = 1000;
        ...
        if (TestNet())
        {
            Send(hSocket, "JOIN #marblecoinTEST\r");
            Send(hSocket, "WHO #marblecoinTEST\r");
        } else
        {
	    LogPrintf("JOIN #Marble\n");
            // randomly join #Marble00-#Marble05
            //int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #marblecoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #marblecoin%02d\r", channel_number).c_str());
        };
