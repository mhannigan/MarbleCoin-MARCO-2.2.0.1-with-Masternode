...
#define MAX_MASTERNODE_COUNT 300

//
// Bootup the masternode, look for a 25000 Marble input and register on the network
//
void CActiveMasternode::ManageStatus()
{
    ...
    if(status == MASTERNODE_NOT_PROCESSED) {
        ...
        LogPrintf("CActiveMasternode::ManageStatus() - Checking inbound connection to '%s'\n", service.ToString().c_str());
          ...
          // MarbleNOTE: There is no logical reason to restrict this to a specific port.  Its a peer, what difference does it make.
...
bool CActiveMasternode::Register(CTxIn vin, CService service, CKey keyCollateralAddress, CPubKey pubKeyCollateralAddress, CKey keyMasternode, CPubKey pubKeyMasternode, std::string &retErrorMessage) {
    ...
    if(vecMasternodes.size() >= MAX_MASTERNODE_COUNT) {
	//too many masternodes already
        retErrorMessage = "Too many masternodes already registered. Limit is 300. Wait and try again later";
        LogPrintf("Too many masternodes already registsred. Limit is 300. Wait and try again later\n");
        return false;
    }
...
bool CActiveMasternode::GetMasterNodeVin(CTxIn& vin, CPubKey& pubkey, CKey& secretKey, std::string strTxHash, std::string strOutputIndex) {
	...
	if(!strTxHash.empty()) {
		...
		bool found = false;
                LogPrintf("CActiveMasternode::GetMasterNodeVin: check suitable inputs\n");
		BOOST_FOREACH(COutput& out, possibleCoins) {
                        std::string outstring = out.ToString();
                        LogPrintf("CActiveMasternode::GetMasterNodeVin: output - %s ",outstring);
			if(out.tx->GetHash() == txHash && out.i == outputIndex)
			{
				...
				break;
                        ...
                        LogPrintf("Bad... Expected hash: %s, output index: %s\n", strTxHash, strOutputIndex);
		}
		if(!found) {
