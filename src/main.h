...
#define START_MASTERNODE_PAYMENTS 1519516800 // 25 Feb 2018, 0:00:00 GMT

#define MASTERNODE_COLLATERAL 25000 //how many coins are required for collateral

static const int64_t DARKSEND_COLLATERAL = (MASTERNODE_COLLATERAL*COIN); //MarbleCoin requires 25,000 coins per MN
static const int64_t DARKSEND_COLLATERAL_DEC = ((MASTERNODE_COLLATERAL-1)*COIN); //MarbleCoin requires 25,000 coins per MN check for -1
static const int64_t DARKSEND_FEE = (0.001*COIN); //increased 
static const int64_t DARKSEND_POOL_MAX = (1111.99*COIN);
static const int REWARD_START = 0;
...
static const int64_t MAX_MONEY =  140000000* COIN; //140 million - MarbleCoin 
...
static const int64_t COIN_YEAR_REWARD = 96 * CENT; // 96% per year for MarbleCoin
...
inline int64_t PastDrift(int64_t nTime) { return nTime - 10 * 60; } // up to 10 minutes in the past
inline int64_t FutureDrift(int64_t nTime) { return nTime + 10 * 60; }

unsigned int GetTargetSpacing(int nHeight);
bool IsProtocolV2(int nHeight);
