...
int nAnonymizeMarbleAmount = 500;
int nLiquidityProvider = 0;
/** Spork enforcement enabled time */
int64_t enforceMasternodePaymentsTime = 4085657524UL;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Marble2
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Marble2
    // Mac: ~/Library/Application Support/Marble2
    // Unix: ~/.Marble2
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Marble2";
#else
    fs::path pathRet;
...
static boost::filesystem::path* pathCached=NULL;
...
const boost::filesystem::path &GetDataDir(bool fNetSpecific)
{
    ...
    if(!pathCached)
        pathCached = new boost::filesystem::path[CChainParams::MAX_NETWORK_TYPES+1];
        for(int i = 0; i <= CChainParams::MAX_NETWORK_TYPES; ++i)
            *(pathCached+i) = boost::filesystem::path("");
...
void createConf()       //Automatic BitcoinDark.conf generation
{
    ...
    const char* nodes =  "\nrpcport=32428"                  //List of known nodes, to be periodically updated
...
{
    std::fill(pathCached, pathCached+CChainParams::MAX_NETWORK_TYPES+1,
              boost::filesystem::path());
}

boost::filesystem::path GetConfigFile()
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "Marble2.conf")); //!!!
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) //Marble2.conf doesn't exist
    {
        createConf();
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "Marble2.pid"));
