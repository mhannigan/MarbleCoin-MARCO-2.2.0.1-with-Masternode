...
int nMagicBlock = 100000; //default value
//#define MAGIC_BLOCK    70000 //where new protocl starts to work
//#define MAGIC_BLOCK    60870 //where new protocol starts to work - DEBUG only

//unsigned int GetTargetSpacing(int nHeight) { return IsProtocolV2(nHeight) ? 180 : 60; } // 60 seconds for old protocol and 3 minutes for MN compatible protocol
unsigned int GetTargetSpacing(int nHeight) { return 60; } // 60 seconds for old protocol 
bool IsProtocolV2(int nHeight) { return nHeight >= nMagicBlock; }

int GetMinPeerProtoVersion(int nHeight)
{
	return(IsProtocolV2(nHeight)? PROTOCOL_VERSION : MIN_PEER_PROTO_VERSION);
}

//unsigned int nTargetSpacing = 60; // 1 minute MarbleCoin
...
const string strMessageMagic = "MarbleCoin Signed Message:\n";
...
bool IsStandardTx(const CTransaction& tx, string& reason)
{
    if (tx.nVersion > CTransaction::CURRENT_VERSION) {
        reason = "version is more new than expected (1)";
        return false;
    }
                        ...
                        bool* pfMissingInputs)
{
    ...
    // Rather not work on nonstandard transactions (unless -testnet)
    //alot of Marble transactions seem non standard, its a bug so we have to accept these, the transactions have still been checekd to be valid and unspent.
    string reason;
...
int64_t GetProofOfWorkReward(int64_t nFees, unsigned int nHeight)
{
    ...
    if (fDebug && GetBoolArg("-printcreation", true))
        printf("GetProofOfWorkReward() : create=%s nSubsidy=%llu\n", FormatMoney(nSubsidy).c_str(), nSubsidy);
...
int64_t GetProofOfStakeRewardMN(int64_t nCoinAge, int64_t nFees, unsigned int nHeight)
{
    int64_t nSubsidy = nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8);
    int64_t nMNReward = GetMasternodePayment(nHeight, nSubsidy);


    LogPrint("creation", "GetProofOfStakeReward(): create=%s nCoinAge=%d\n", FormatMoney(nSubsidy), nCoinAge);

    return nSubsidy + nFees + nMNReward;
}
...
static unsigned int GetNextTargetRequiredV1(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    unsigned int nTargetSpacing = GetTargetSpacing(pindexPrev->nHeight);
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
	...
	nInputs += tx.vin.size();
        ...
        else
        {
            ...
            /// masternode payment check!!!! - start
            int64_t masternodePaymentValue = GetMasternodePayment(pindex->nHeight, 0);

            if(masternodePaymentValue != 0) {
            ...
            bool bFind = masternodePayments.GetBlockPayee(pindex->nHeight, payee);
            if(fDebug) {
            if(!bFind)
                LogPrintf("ConnectBlock: cannot find masternode for block %d!\n", pindex->nHeight);
            else                    
		LogPrintf("ConnectBlock: Found masternode for block %d, payee %s\n", pindex->nHeight, payee.ToString());
	    }
            ...
            if(fDebug)
               	LogPrintf("ConnectBlock: Enumerate payments to find masternode payments...\n");

            for (unsigned int i = 0; i < tx.vout.size(); i++) {
                   if(tx.vout[i].nValue == masternodePaymentValue )
                       foundPaymentAmount = true;
                   if(tx.vout[i].scriptPubKey == payee )
                       foundPayee = true;
                   if(tx.vout[i].nValue == masternodePaymentValue && tx.vout[i].scriptPubKey == payee)
                       foundPaymentAndPayee = true;
            }
            /*
            if(!foundPaymentAndPayee) {
                ...
                if(fDebug) { LogPrintf("ConnectBlock : Couldn't find masternode payment(%d|%d) or payee(%d|%s) nHeight %d. \n", masternodePaymentValue, foundPayee, address2.ToString().c_str(), pindex->nHeight); 
                    return DoS(100, error("ConnectBlock : Couldn't find masternode payment or payee"));
                } else {
                    return DoS(100, error("ConnectBlock : Couldn't find masternode payment or payee"));
                }
                //decrease out value by masternode payment to satisfy con stake check
                nStakeReward -= masternodePaymentValue;

                if(fDebug) { 
                    LogPrintf("ConnectBlock : Found masternode payment %d\n", pindex->nHeight); 
                }
           } */
           }

            /// masternode payment check!!!! - end
    ...
    if (IsProofOfStake())
    {
        ...
        int64_t nCalculatedStakeRewardMN = GetProofOfStakeRewardMN(nCoinAge, nFees, pindex->nHeight);

        if (nStakeReward > max(nCalculatedStakeReward,nCalculatedStakeRewardMN))
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProof)
{
    ...
    if (!CheckStakeModifierCheckpoints(pindexNew->nHeight, pindexNew->nStakeModifierChecksum))
        return error("AddToBlockIndex() : Rejected by stake modifier checkpoint height=%d, modifier=%lu", pindexNew->nHeight, nStakeModifier);
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    if (IsProofOfStake())
    {
        ...
        if (!CheckCoinStakeTimestamp(GetBlockTime(), (int64_t)vtx[1].nTime))
            return DoS(50, error("CheckBlock() : coinstake timestamp violation nTimeBlock=%lu, nTimeTx=%u", GetBlockTime(), vtx[1].nTime));
...
// ----------- instantX transaction scanning -----------
    //Disable spork check for masternode
    ...
    if(nTime > START_MASTERNODE_PAYMENTS && IsProtocolV2(pindexBest->nHeight))
    ...
    if(MasternodePayments)
    {
        ...
        //LogPrintf("Process masternode payments\n");
        ...
        if(pindex != NULL){
            if(pindex->GetBlockHash() == hashPrevBlock){
                ...
                if (!fIsInitialDownload && pindex != NULL)
                {
                    ...
                    bool bFind = masternodePayments.GetBlockPayee(pindexBest->nHeight+1, payee);
                    if(fDebug) {
                    if(!bFind)
			LogPrintf("main.cpp: cannot find masternode for block %d!\n", pindexBest->nHeight+1);
                    else                    
			LogPrintf("main.cpp: Found masternode for block %d, payee %s\n", pindexBest->nHeight+1, payee.ToString());
		    }
                    ...
                    if(fDebug)
                    	LogPrintf("Enumerate payments to find masternode payments...\n");
...
bool CBlock::AcceptBlock()
{
    ...
    unsigned int uNextTarget = GetNextTargetRequired(pindexPrev, IsProofOfStake());
    
    if (nBits != uNextTarget) {
		LogPrintf("AcceptBlock() : nBits=%u, nextTarget=%u\n",nBits,uNextTarget);
    ...
    if (IsProofOfStake())
    {
        ...
        {
            LogPrintf("AcceptBlock() : block height is %d\n", nHeight);
...
if(!IsInitialBlockDownload()){
        ...
        if (!fLiteMode && !fImporting && !fReindex && pindexBest->nHeight > Checkpoints::GetTotalBlocksEstimate()){
            ...
            if(masternodePayments.GetBlockPayee(pindexBest->nHeight, payee, vin)){
                ...
                CMasterNode* pmn = GetPrevMasternodeObjectByVin(vin);
...
bool CBlock::SignBlock(CWallet& wallet, int64_t nFees)
{
    ...
    if (!vtx[0].vout[0].IsEmpty()) {
        if(fDebug)
            LogPrintf("CBlock::SignBlock vtx[0].vout[0].IsEmpty() - true, exit\n");
        return false;
    }
    ...
    if (IsProofOfStake()) {
        if(fDebug)
            LogPrintf("CBlock::SignBlock trying to sign a complete PoS block, exit\n");
        return true;
    }
    ...
    if (nSearchTime > nLastCoinStakeSearchTime)
    {
       ...
       //if (wallet.CreateCoinStake(wallet, nBits, 1000, nFees, txCoinStake, key)) //DEBUG!!!
        {
            if(fDebug)
                LogPrintf("CBlock::SignBlock create new coin stake!\n");
...
{
    if(nHeight < nMagicBlock)
		return 0;
    else if (nHeight >= nMagicBlock && nHeight <150000)
		return 50  * COIN;
		...
		int64_t ret = static_cast<int64_t>( 25.0 * COIN * (500000.0/nHeight));
		return ret;
	}
