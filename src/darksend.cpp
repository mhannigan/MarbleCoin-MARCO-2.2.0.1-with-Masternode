...
void ProcessMessageDarksend(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "dsf") { //DarkSend Final tx
	    ...
	    if(!AcceptableInputs(mempool, tx, false, &fMissingInputs)){
    ...
    if(fMasterNode) {
        ...
        BOOST_FOREACH(const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat Marble and make it unusable. To
                stop these kinds of attacks 1 in 50 successful transactions are charged. This
                adds up to a cost of 0.002Marble per transaction on average.
            */
            if(r <= 20)
            {
...
bool CDarkSendPool::IsCollateralValid(const CTransaction& txCollateral){
    ...
    bool fMissingInputs = false;
    if(!AcceptableInputs(mempool, txCollateral, false, &fMissingInputs)){
...
bool CDarkSendPool::DoAutomaticDenominating(bool fDryRun, bool ready)
{
    ...
    int64_t nBalanceNeedsAnonymized = nAnonymizeMarbleAmount*COIN - pwalletMain->GetAnonymizedBalance();
...
//        }
    ...
    if(!sessionFoundMasternode){
        ...
        double fMarbleSubmitted = (sessionTotalValue / CENT);
        LogPrintf("Submitting Darksend for %f Linda CENT - sessionTotalValue %d\n", fMarbleSubmitted, sessionTotalValue);
        ...
        if(c % 60*(nLiquidityProvider+1) == 0){
            if(nLiquidityProvider!=0){
                ...
                if(nRand == 50+nLiquidityProvider && pwalletMain->GetAverageAnonymizedRounds() > 8){
                    ...
                    nAnonymizeMarbleAmount = (rand() % nLeftToAnon)+3;
                } else {
